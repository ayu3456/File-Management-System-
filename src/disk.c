...
bool init_disk() {
    ...
    if (disk_fp == NULL) {
        ...
        format_disk();
        printf("Virtual disk created and formatted: disk.txt\n");
    } else {
        // If disk.txt exists, load the file system
        load_file_system();
        printf("Virtual disk loaded from disk.txt\n");
    }
    return true;
}
...
void format_disk() {
    ...
    int metadata_blocks_count = (DATA_BLOCKS_OFFSET + BLOCK_SIZE - 1) / BLOCK_SIZE; // Total blocks for metadata
    if (metadata_blocks_count >= TOTAL_BLOCKS) { // Ensure we don't try to allocate beyond disk size
...
void write_block(int block_num, const char *data) {
    ...
    if (actual_offset < DATA_BLOCKS_OFFSET || actual_offset + BLOCK_SIZE > VIRTUAL_DISK_SIZE) {
        fprintf(stderr, "Error: Invalid block write access (offset: %ld, block_num: %d)\n", actual_offset, block_num);
        return;
    }
...
void read_block(int block_num, char *buffer) {
    ...
    if (actual_offset < DATA_BLOCKS_OFFSET || actual_offset + BLOCK_SIZE > VIRTUAL_DISK_SIZE) {
        fprintf(stderr, "Error: Invalid block read access (offset: %ld, block_num: %d)\n", actual_offset, block_num);
        return;
    }
    fseek(disk_fp, actual_offset, SEEK_SET);
...
int find_contiguous_free_blocks(int count) {
    ...
    int first_data_block_logical_idx = (DATA_BLOCKS_OFFSET + BLOCK_SIZE - 1) / BLOCK_SIZE;

    for (int i = first_data_block_logical_idx; i < TOTAL_BLOCKS; i++) {
        if (!block_bitmap[i]) {
            // Block is free
            if (current_contiguous_count == 0) {
                start_block_idx = i; // Mark the start of a potential contiguous block run
            }
...
void load_file_system() {
    // OS Concept: Metadata Handling - Reading superblock, file table, and directory from disk
...
void save_file_system() {
    // OS Concept: Metadata Handling - Writing superblock, file table, and directory to disk
    fseek(disk_fp, SUPERBLOCK_OFFSET, SEEK_SET);
...
void show_disk_status() {
    printf("\nDisk Status:\n");
    printf("Total Disk Size: %d bytes (%.2f MB)\n", superblock.total_disk_size, (float)superblock.total_disk_size / (1024 * 1024));
