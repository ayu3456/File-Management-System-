...
// Create a new file
bool create_file(const char *filename, int size, bool read_perm,
                 bool write_perm) {
  ...
  if (num_blocks_needed > 0) {
    ...
    if (start_block == -1) {
      fprintf(stderr,
              "Error: Not enough contiguous disk space for '%s' (size: %d "
              "bytes, blocks needed: %d).\n",
  ...
  save_file_system();
  fprintf(stderr,
          "File '%s' created successfully (size: %d bytes, blocks: %d).\n",
          filename, size, num_blocks_needed);
  return true;
}
...
bool write_file(const char *filename, const char *data) {
  ...
  if (new_num_blocks != fte->num_blocks) {
    ...
    if (new_num_blocks > 0) {
      ...
      if (new_start_block == -1) {
        fprintf(stderr,
                "Error: Not enough contiguous disk space to write all data to "
                "'%s'.\n",
                filename);
  ...
  for (int i = 0; i < fte->num_blocks; i++) {
    ...
    int bytes_to_write =
        (data_len - offset > BLOCK_SIZE) ? BLOCK_SIZE : (data_len - offset);
  ...
  save_file_system();
  fprintf(stderr,
          "Data written to '%s'. (New size: %d bytes, New blocks: %d)\n",
...
bool read_file(const char *filename, char *buffer, int buffer_size) {
  ...
  if (buffer_size < fte->file_size + 1) { // +1 for null terminator
    fprintf(stderr,
            "Error: Buffer too small to read entire file '%s'. Required: %d, "
            "Available: %d.\n",
  ...
  for (int i = 0; i < fte->num_blocks; i++) {
    ...
    int bytes_to_copy = (fte->file_size - bytes_read > BLOCK_SIZE)
                            ? BLOCK_SIZE
                            : (fte->file_size - bytes_read);
  ...
  // printf("File content:\n%s\n", buffer); // Moved to main.c for interactive
  // mode or JSON output
  return true;
}
